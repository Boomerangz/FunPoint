...
public class BalloonOverlayView<Item extends OverlayItem> extends FrameLayout {
	...
	protected void setBalloonData(Item item, ViewGroup parent) {
		if (item.getTitle() != null) {
	        ...
	        Map.balloon_item_title.setText(item.getTitle());
	        Map.balloon_item_title.setVisibility(VISIBLE);
		} else {
			title.setText("");
			title.setVisibility(GONE);
			
	        Map.balloon_item_title.setText("");
	        Map.balloon_item_title.setVisibility(GONE);
		}
		if (item.getSnippet() != null) {
			snippet.setVisibility(VISIBLE);
			snippet.setText(item.getSnippet());
			
	        Map.balloon_item_snippet.setVisibility(VISIBLE);
	        Map.balloon_item_snippet.setText(item.getSnippet());
		} else {
			snippet.setText("");
			snippet.setVisibility(GONE);
			
	        Map.balloon_item_snippet.setVisibility(GONE);
	        Map.balloon_item_snippet.setText("");
		}
	}
