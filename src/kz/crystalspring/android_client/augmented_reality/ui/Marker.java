...
import kz.crystalspring.android_client.augmented_reality.data.PhysicalLocation;
//import kz.crystalspring.android_client.augmented_reality.ui.objects.PaintableBox;
import kz.crystalspring.android_client.augmented_reality.ui.objects.PaintableBoxedIconAndText;
//import kz.crystalspring.android_client.augmented_reality.ui.objects.PaintableBoxedText;
//import kz.crystalspring.android_client.augmented_reality.ui.objects.PaintableGps;
//import kz.crystalspring.android_client.augmented_reality.ui.objects.PaintableIcon;
import kz.crystalspring.android_client.augmented_reality.ui.objects.PaintableObject;
...
public class Marker implements Comparable<Marker> {
    ...
    public float fAngle = 0; // Added by EArt 02 11 2011

    private float initialY = 0.0f;
    
    private volatile static CameraModel cam = null;

    private volatile PaintableBoxedIconAndText theBox = null;
    private volatile PaintablePosition boxContainer = null;
...
//    private static PaintablePosition touchPosition = null;
    private String fId = ""; // added by EArt 04 12 2011
    private float fScale = 1; // added by EArt 04 12 2011
    private Bitmap bitmap = null;
    
//    private float fX = 0;
//    private float fY = 0;
//    private float fA = 0;
//	private static final float C_FILTER_FACTOR = (float) 0.6;
    
    
	public Marker(String name, double latitude, double longitude, double altitude, int color, Bitmap bitmap, String pId) {
		set(name, latitude, longitude, altitude, color, bitmap, pId);
	}
	 ...
	 */
	public synchronized void set(String name, double latitude, double longitude, double altitude, int color, Bitmap bitmap, String pId) {
		...
		this.initialY = 0.0f;
		this.fId = pId; // added by EArt 04 12 2011
		this.bitmap = bitmap;
	}
	
	// added by EArt 04 12 2011
	public String GetId () {
		return fId;
	}
    ...
    public synchronized Vector getScreenPosition() {
        ...
        if (theBox!=null) y += (theBox.getHeight()/2);
    ...
    public synchronized float getHeight() {
        if (boxContainer!=null) return boxContainer.getHeight();    	
        if (symbolContainer==null || boxContainer==null) return 0f;
        return symbolContainer.getHeight()+boxContainer.getHeight();
    }
    
    public synchronized float getWidth() {
        if (boxContainer!=null) return boxContainer.getWidth();
        if (symbolContainer==null || boxContainer==null) return 0f;
        float w1 = boxContainer.getWidth();
	...
	public synchronized boolean isPointOnMarker(float x, float y) {
        ...
        float myX = screenPositionArray[0] + getWidth()/2 - bitmap.getWidth() - 8;
        float myY = screenPositionArray[1];
        float adjWidth = getWidth()/2 + bitmap.getWidth() + 8;
...
//        drawIcon(canvas); !!EArt 
//        drawText(canvas); //EArt
        drawBox(canvas);
...
//        C_Log.v(3,"touchBox", "ul (x="+(adjX)+" y="+(adjY)+")");
//        C_Log.v(3,"touchBox", "ur (x="+(adjX+width)+" y="+(adjY)+")");
//        C_Log.v(3,"touchBox", "ll (x="+(adjX)+" y="+(adjY+height)+")");
//        C_Log.v(3,"touchBox", "lr (x="+(adjX+width)+" y="+(adjY+height)+")");


//if (touchBox==null) touchBox = new PaintableBox(fwt,fht);
//else touchBox.set(fwt,fht);
//
//if (touchPosition==null) touchPosition = new PaintablePosition(touchBox, fxt, fyt, fAngle, 1);
//else touchPosition.set(touchBox, fxt, fyt, fAngle, 1);
//        
...
//        symbolXyzRelativeToCameraView.get(symbolArray);
//        
//        fAngle = Utilities.getAngle(symbolArray[0], symbolArray[1], textArray[0], textArray[1]) + 90; // Added by EArt 02 11 2011
//        
//        if (symbolContainer==null) symbolContainer = new PaintablePosition(gpsSymbol, symbolArray[0], symbolArray[1], fAngle, fScale);
//        else symbolContainer.set(gpsSymbol, symbolArray[0], symbolArray[1], fAngle, fScale);
...
//	    if (textBox==null) textBox = new PaintableBoxedIconAndText(textStr, Math.round(maxHeight / 2f) + 1, 300);
...
//	    fAngle = currentAngle + 90; // EArt, old: float angle = ...
//	    if (textContainer==null) textContainer = new PaintablePosition(textBox, x, y, fAngle, fScale);
//	    else textContainer.set(textBox, x, y, fAngle, fScale);
//	    textContainer.paint(canvas);
//	}

    
    private synchronized void drawBox(Canvas canvas) {
	    ...
	    symbolXyzRelativeToCameraView.get(symbolArray);
	    float maxHeight = Math.round(canvas.getHeight() / 10f) + 1;
	    if (theBox==null) theBox = new PaintableBoxedIconAndText(textStr, Math.round(maxHeight / 2f) + 1, 220, bitmap);
	    else theBox.set(textStr, Math.round(maxHeight / 2f) + 1, 220);
	    
	    float x = textArray[0] - theBox.getWidth() / 2;
	    float y = textArray[1] ; // +maxHeight
	    float currentAngle = Utilities.getAngle(symbolArray[0], symbolArray[1], textArray[0], textArray[1]);
	    fAngle = currentAngle + 90; // EArt, old: float angle = ...
...
//	    if (fY==0) fY = x;
//	    if (fA==0) fA = fAngle;
//	    fX = x*C_FILTER_FACTOR + fX*(1-C_FILTER_FACTOR);
//	    fY = y*C_FILTER_FACTOR + fY*(1-C_FILTER_FACTOR);
//	    fA = fAngle*C_FILTER_FACTOR + fA*(1-C_FILTER_FACTOR);
//	    if (boxContainer==null) {
//	    	boxContainer = new PaintablePosition(theBox, fX, fY, fAngle, fScale);
//	    } else { 
//	    	boxContainer.set(theBox, fX, fY, fAngle, fScale);
//	    }
	    
	    if (boxContainer==null) {
	    	boxContainer = new PaintablePosition(theBox, x, y, fAngle, fScale);
	    } else { 
	    	boxContainer.set(theBox, x, y, fAngle, fScale);
	    }
