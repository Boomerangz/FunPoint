...
public class SensorsActivity extends Activity implements SensorEventListener, LocationListener {
    ...
    private static final float C_GRAV_FILTER_FACTOR = (float) 0.9; // процент от нового показания к старому
    private static final float C_MAGN_FILTER_FACTOR = (float) 0.6; // процент от нового показания к старому
    ...
    public static boolean fLocationUpdateNeeded = true;
    ...
    public void onStart() {
        ...
        try {
            ...
            if (fLocationUpdateNeeded) { // нужно или нет автообновление центральной позиции
            	...
            	try {
            		...
            		if(gps!=null)
            			...
            			onLocationChanged(ARData.fHardFix);
            	} catch (Exception ex2) {
            		onLocationChanged(ARData.fHardFix);
            	}
            }
    ...
    public void onSensorChanged(SensorEvent evt) {
        ...
        if (evt.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            grav[0] = evt.values[0] * C_GRAV_FILTER_FACTOR + grav[0] * (1-C_GRAV_FILTER_FACTOR);
            grav[1] = evt.values[1] * C_GRAV_FILTER_FACTOR + grav[1] * (1-C_GRAV_FILTER_FACTOR);
            grav[2] = evt.values[2] * C_GRAV_FILTER_FACTOR + grav[2] * (1-C_GRAV_FILTER_FACTOR);
        } else if (evt.sensor.getType() == Sensor.TYPE_MAGNETIC_FIELD) {
            mag[0] = evt.values[0] * C_MAGN_FILTER_FACTOR + mag[0] * (1-C_MAGN_FILTER_FACTOR);
            mag[1] = evt.values[1] * C_MAGN_FILTER_FACTOR + mag[1] * (1-C_MAGN_FILTER_FACTOR);
            mag[2] = evt.values[2] * C_MAGN_FILTER_FACTOR + mag[2] * (1-C_MAGN_FILTER_FACTOR);
        }
    ...
    public void onAccuracyChanged(Sensor sensor, int accuracy) {
        ...
        if(sensor.getType() == Sensor.TYPE_MAGNETIC_FIELD && accuracy==SensorManager.SENSOR_STATUS_UNRELIABLE) {
        	C_Log.v(2, TAG, "Compass data unreliable");
        }
    }
}
