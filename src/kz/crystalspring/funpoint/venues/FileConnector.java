...
public class FileConnector
{
	...
	private static final BasicNameValuePair key_pair = new BasicNameValuePair("key", JAM_KEY);
	...
	public static JSONObject getJSONObjFromText(String wantedID, String JSONString)
	{
		...
		{
			Toast.makeText(context.getApplicationContext(), "ошибка парсинга", Toast.LENGTH_SHORT).show();
			returnObject = null;
		}
		return returnObject;
	}
	...
	public static HashSet<String> getJamCinemaList()
	{
		...
		try
		{
			...
			byte[] vBytes;
			File cinemaListFile=new File(context.getFilesDir() + "/" + "cinema_list.json");
			if ((cinemaListFile.exists()&&cinemaListFile.canRead()))
			{
				vBytes = C_FileHelper.ReadFile(new File(context.getFilesDir() + "/" + "cinema_list.json"));
			}
			else
			{
				InputStream is=context.getAssets().open("cinema_list.json");
				vBytes = new byte[is.available()];
				is.read(vBytes);
			}
	...
	public static JSONObject loadCinemaInfo(String FsqId) throws Exception
	{
		List<BasicNameValuePair> params = new ArrayList<BasicNameValuePair>();
		params.add(new BasicNameValuePair("fsq_id", FsqId));
		params.add(key_pair);
		String sResponse = HttpHelper.getInstance().loadPostByUrl(JAM_CINEMA_URL, params);
	...
	public static JSONObject loadJSONEventById(String id)
	{
		...
		params.add(key_pair);
		String sResponse = HttpHelper.getInstance().loadPostByUrl(JAM_EVENT_URL, params);
		try
		{
			return new JSONObject(sResponse);
	...
	public static JSONArray loadJSONCinemaEventsList()
	{
		...
		params.add(key_pair);
		String sResponse = HttpHelper.getInstance().loadPostByUrl(JAM_EVENTS_LIST_URL, params);
	...
	public static JSONArray loadJSONPlaceList(String event_id)
	{
		...
		params.add(key_pair);
		String sResponse = HttpHelper.getInstance().loadPostByUrl(JAM_PLACES_LIST_URL, params);
